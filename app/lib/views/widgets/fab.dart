...
class _FabState extends State<Fab> with SingleTickerProviderStateMixin {
  ...
  initState() {
    ...
    _buttonColor = ColorTween(begin: Colors.blue, end: Colors.red).animate(CurvedAnimation(
      parent: _animationController,
      curve: Interval(0.00, 1.00, curve: Curves.linear),
    ));
    _translateButton = Tween<double>(begin: _fabHeight, end: -14).animate(CurvedAnimation(
      parent: _animationController,
      curve: Interval(0.0, 0.75, curve: _curve),
    ));
    _toggleButton = Tween<double>(begin: 0, end: Math.pi/4).animate(CurvedAnimation(
      parent: _animationController, 
      curve: Interval(0, 1, curve: _curve)
    ));
  }
      ...
      children: <Widget>[
        FadeTransition(
          child: Container(
            width: 260,
            height: 40,
            child: Text(name, textAlign: TextAlign.right, style: TextStyle(
              fontSize: 20, 
        ...
        isOpened
            ? Positioned(right: 0, left: 0, top: 0, bottom: 0,
              child: FadeTransition(
                opacity: _animateIcon,
                child: Image.asset('assets/images/Agate.png', fit: BoxFit.cover),
              ),
            ) : SizedBox(),
        isOpened
            ? Positioned(right: 0, left: 0, top: 0, bottom: 0,
